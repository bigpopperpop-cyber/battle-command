      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
        {tab === 'INFO' ? (
          <>
            {isPlanet && isMine && (
              <div className="flex gap-2">
                <input value={newName} onChange={(e) => setNewName(e.target.value)} placeholder="Rename colony..." className="flex-1 bg-black/40 border-none rounded-lg px-3 text-[10px] outline-none text-white" />
                <button onClick={() => { onIssueOrder('RENAME_PLANET', { name: newName }); setNewName(''); }} className="px-3 py-2 bg-cyan-600 rounded-lg text-[8px] font-black uppercase">Apply</button>
              </div>
            )}
            {isPlanet ? (
              <div className="grid grid-cols-3 gap-2">
                {['Pop', 'Mines', 'Fact'].map((label, i) => (
                  <div key={label} className="bg-slate-950/60 p-2 rounded-xl border border-white/5 text-center">
                    <span className="block text-[7px] font-black text-slate-600 uppercase">{label}</span>
                    <span className="text-xs font-bold text-white">{(i === 0 ? selection.population.toFixed(1) : i === 1 ? selection.mines : selection.factories)}</span>
                  </div>
                ))}
              </div>
            ) : (
              <div className="bg-slate-950/60 p-3 rounded-xl border border-white/5">
                ...
                <div className="h-1.5 bg-slate-900 rounded-full overflow-hidden"><div className="h-full bg-emerald-500" style={{ width: `${(selection.hp/selection.maxHp)*100}%` }} /></div>
              </div>
            )}
            {isMine && (
              <div className="space-y-4">
                ...
                <button onClick={() => onIssueOrder(isPlanet ? 'BUILD_SHIP' : 'SET_COURSE', { type: 'WARSHIP' })} className="w-full py-4 bg-cyan-600 rounded-xl font-black text-[10px] uppercase shadow-xl">{isPlanet ? 'Build Warship' : isSettingCourse ? '📡 Tap Target' : '🚀 Set Course'}</button>
              </div>
            )}
          </>
        ) : (
            ...
            {Object.entries(techLevels).map(([t, level]) => {
              ...
              return (
                <div key={t} className="p-3 bg-slate-900/60 rounded-xl border border-white/5 flex justify-between items-center">
                  <div>
                    <p className="text-[10px] font-black text-white uppercase">{t} LVL {level}</p>
                    <p className="text-[8px] text-slate-500">{t === 'engine' ? '+10% Speed' : t === 'shields' ? '+10% First Hit Def' : '+10% Scan Range'}</p>
                  </div>
                  <button onClick={() => onIssueOrder('RESEARCH_TECH', { tech: t })} disabled={credits < cost} className="px-3 py-2 bg-emerald-600/20 text-emerald-400 border border-emerald-500/20 rounded-lg text-[8px] font-black uppercase disabled:opacity-20">UP: {cost}</button>
                </div>
              );
            })}
          </div>
