    ...
    >
      ...
      <div className="absolute transition-transform duration-75" style={{ transform: `translate3d(${offset.x}px, ${offset.y}px, 0) scale(${zoom})`, width: GRID_SIZE, height: GRID_SIZE, transformOrigin: '0 0' }}>
          ...
          {combatEvents.map(ev => (
            <g key={ev.id}>
              <line 
                ...
                stroke={ev.color} strokeWidth="4" strokeLinecap="round" 
                style={{ animation: 'laser-pulse 0.2s infinite ease-in-out', filter: 'drop-shadow(0 0 5px currentColor)' }} 
              />
              <circle cx={ev.attackerPos?.x} cy={ev.attackerPos?.y} r="8" fill={ev.color} style={{ animation: 'spark 0.3s forwards' }} />
        ...
        {activeEvents.map((e, idx) => e.type === 'COMET' && (
          <div key={`comet-${idx}`} className="absolute -translate-x-1/2 -translate-y-1/2 w-12 h-12 bg-white rounded-full blur-xl animate-pulse" style={{ left: e.x, top: e.y }} />
        ))}

        {combatScraps.filter(s => Date.now() - s.timestamp < 3000).map(s => (
          <div key={s.id} className="absolute -translate-x-1/2 -translate-y-1/2 w-32 h-32 border-2 border-orange-500 rounded-full" style={{ left: s.x, top: s.y, animation: 'shockwave 1s forwards' }} />
        ))}
        ...
        {planets.map(p => {
          ...
          return (
            <div key={p.id} onPointerUp={(e) => { 
              ...
              <div className={`w-14 h-14 rounded-full border-2 transition-all flex items-center justify-center ${isSelected ? 'scale-110 border-white shadow-[0_0_20px_white]' : 'border-white/10'}`} style={{ backgroundColor: PLAYER_COLORS[p.owner] }}>
        ...
        {ships.map(s => {
          ...
          return (
            <div key={s.id} onPointerUp={(e) => { 
              ...
              <div className={`w-8 h-8 border-2 rotate-45 flex items-center justify-center bg-slate-900 transition-transform ${selectedId === s.id ? 'scale-150 border-white shadow-[0_0_15px_currentColor]' : ''} ${s.isScrambled ? 'opacity-30 blur-sm' : ''}`} style={{ borderColor: PLAYER_COLORS[s.owner], color: PLAYER_COLORS[s.owner] }}>
      ...
      <div className="absolute bottom-6 right-6 flex flex-col gap-2 z-50">
        <button onClick={() => setZoom(z => Math.min(2, z + 0.15))} className="w-10 h-10 bg-slate-900 rounded-xl font-bold flex items-center justify-center border border-white/10">+</button>
        <button onClick={() => setZoom(z => Math.max(0.1, z - 0.15))} className="w-10 h-10 bg-slate-900 rounded-xl font-bold flex items-center justify-center border border-white/10">-</button>
      </div>
    </div>
  );
};
