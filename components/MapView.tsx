...
const MapView: React.FC<MapViewProps> = ({ 
  ...
  return (
    <div 
      ...
      onPointerDown={(e) => { isDragging.current = true; moved.current = false; startPos.current = { x: e.clientX, y: e.clientY }; startOffset.current = { ...offset }; (e.target as HTMLElement).setPointerCapture(e.pointerId); }}
      onPointerMove={(e) => { if (!isDragging.current) return; const dx = e.clientX - startPos.current.x; const dy = e.clientY - startPos.current.y; if (Math.abs(dx) > 10 || Math.abs(dy) > 10) moved.current = true; setOffset({ x: startOffset.current.x + dx, y: startOffset.current.y + dy }); }}
      onPointerUp={(e) => { isDragging.current = false; (e.target as HTMLElement).releasePointerCapture(e.pointerId); }}
    >
      <style>{`
        @keyframes laser-grow { 0% { opacity: 0; } 50% { opacity: 1; } 100% { opacity: 0; } }
        @keyframes scrap-fade { 0% { transform: scale(1); opacity: 1; } 100% { transform: scale(3); opacity: 0; } }
      ...
      <div className="absolute transition-transform duration-75" style={{ transform: `translate3d(${offset.x}px, ${offset.y}px, 0) scale(${zoom})`, width: GRID_SIZE, height: GRID_SIZE, transformOrigin: '0 0' }}>
        <svg className="absolute inset-0 w-full h-full pointer-events-none z-10">
          {combatEvents.map(ev => <line key={ev.id} x1={ev.attackerPos?.x} y1={ev.attackerPos?.y} x2={ev.targetPos?.x} y2={ev.targetPos?.y} stroke={ev.color} strokeWidth="3" strokeDasharray="5,5" style={{ animation: 'laser-grow 0.5s infinite' }} />)}
        </svg>
        ...
        {combatScraps.filter(s => Date.now() - s.timestamp < 3000).map(s => (
          <div key={s.id} className="absolute -translate-x-1/2 -translate-y-1/2 w-20 h-20 border-2 border-orange-500 rounded-full" style={{ left: s.x, top: s.y, animation: 'scrap-fade 1.5s forwards' }} />
        ))}
        ...
        {planets.map(p => {
          ...
          return (
            <div key={p.id} onClick={(e) => { e.stopPropagation(); if (!moved.current) onSelect(p.id); }} className="absolute -translate-x-1/2 -translate-y-1/2 flex flex-col items-center p-24 z-20" style={{ left: p.x, top: p.y }}>
              ...
              <div className={`w-14 h-14 rounded-full border-2 transition-all flex items-center justify-center ${isSelected ? 'scale-110 border-white' : 'border-white/10'}`} style={{ backgroundColor: PLAYER_COLORS[p.owner] }}>
        ...
        {ships.map(s => {
          ...
          return (
            <div key={s.id} onClick={(e) => { e.stopPropagation(); if (!moved.current) onSelect(s.id); }} className="absolute -translate-x-1/2 -translate-y-1/2 cursor-pointer p-8 z-40" style={{ left: pos.x, top: pos.y }}>
              <div className={`w-8 h-8 border-2 rotate-45 flex items-center justify-center bg-slate-900 ${selectedId === s.id ? 'scale-150 border-white' : ''} ${s.isScrambled ? 'opacity-30 blur-sm' : ''}`} style={{ borderColor: PLAYER_COLORS[s.owner] }}>
      ...
      <div className="absolute bottom-6 right-6 flex flex-col gap-2 z-50">
        <button onClick={() => setZoom(z => Math.min(2, z + 0.15))} className="w-10 h-10 bg-slate-900 rounded-xl font-bold flex items-center justify-center text-xl">+</button>
        <button onClick={() => setZoom(z => Math.max(0.1, z - 0.15))} className="w-10 h-10 bg-slate-900 rounded-xl font-bold flex items-center justify-center text-xl">-</button>
      </div>
    </div>
  );
};
